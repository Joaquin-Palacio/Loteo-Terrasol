  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!/^\d{10}$/.test(phoneInput)) {
      responseDiv.innerHTML =
        '<div class="alert alert-danger">Ingrese un número de teléfono válido a 10 dígitos</div>';
      return;
    }
    ...
    emailjs
      ...
      .catch((err) => {
        btn.value = 'Enviar Email';
        responseDiv.innerHTML =
          '<div class="alert alert-danger">Error al enviar el mensaje. Por favor, inténtelo de nuevo más tarde.</div>';
      });
  });
});
